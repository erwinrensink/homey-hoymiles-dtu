...
const ModbusExplorer = require('./explorer');
...
class HoymilesClient {
  ...
  constructor(host, port = 502, unitId = 1) {
    ...
    this.explorer = new ModbusExplorer(this);
  }

  async scanRegisters() {
    await this.explorer.scan(
      0x1000,
      0x1800,
      100
    );
...
async readRealtimeData() {
  ...
  for (const startAddress of startAddresses) {
    ...
    console.log(`Lezen vanaf 0x${startAddress.toString(16).toUpperCase()}`);

    try {

      const result = await this.readHoldingRegisters(startAddress, blockSize);
      ...
      result.data.forEach((value, index) => {
        ...
        console.log(
          `0x${address.toString(16).toUpperCase().padStart(4, '0')} = ${value}`
        );

      });

      allRegisters.push(...result.data);

    } catch (err) {

      console.log(`Adres 0x${startAddress.toString(16).toUpperCase()} bestaat niet.`);
      break;
